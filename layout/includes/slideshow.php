...
$numberofslides = theme_academi_kair_get_setting('numberofslides');

$slideimage = array();
for ($s1 = 1; $s1 <= $numberofslides; $s1++) {
    $slideimage [] = theme_academi_kair_render_slideimg($s1, 'slide' . $s1 . 'image');
}
...
if ($countslideimage) {
    ...
    <div class="carousel-inner" role="listbox">
        ...
        <?php for ($s1 = 1; $s1 <= $numberofslides; $s1++) :
                $slidecaption = theme_academi_kair_get_setting('slide' . $s1 . 'caption', true);
                $slidedesc = theme_academi_kair_get_setting('slide' . $s1 . 'desc', 'format_html');

                $slideimg = theme_academi_kair_render_slideimg($s1, 'slide' . $s1 . 'image');
            if ($slideimg) {
                    $visableslide += 1;
